...
__all__ = ['CometCallback', 'EpochTimer']
    ...
    *Note*: ``PrintLog`` will not result in good outputs if the number
    ...
    def format_row(self, row, key, color):
        ...
        if (key_best in row) and row[key_best]:
            template = color + template + Color.ENDC.value
    ...
    def _yield_keys_formatted(self, row):
        colors = cycle([color.value for color in Color if color != color.ENDC])
...
class ProgressBar(Callback):
    ...
    def on_epoch_begin(self, net, dataset_train=None, dataset_valid=None, **kwargs):
        ...
        if self._use_notebook():
            self.pbar_ = tqdm_notebook(total=batches_per_epoch, leave=False)
        else:
            self.pbar_ = tqdm(total=batches_per_epoch, leave=False)
