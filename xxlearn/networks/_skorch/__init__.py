...
from xxlearn.utils import doc_string, to_tensor, _is_slicedataset
from xxlearn.callback import EpochScoring, PassthroughScoring, EpochTimer, PrintLog, LossEpoch
from .data import *
...
def get_accelerator(module, kwargs):
    ...
    from .hf import AccelerateMixin
...
class Classifier(NeuralNetClassifier):
    ...
    def initialize(self):
        self.__pre_init__()
        self.set_params(iterator_valid__collate_fn = self.iterator_train__collate_fn)
    ...
    def get_bestscore(self, metric, direction='maximize'):
        hist = self.history
        assert direction == 'maximize', "i haven't figured out how to find minimum"
        try:
            valid_test = hist[:, ('valid_%s'%metric, 'epoch', 'test_%s'%metric)]
        except KeyError:
            valid_test = hist[:, ('valid_%s'%metric, 'epoch')]
        dir = -1 if direction == 'maximize' else 1
        test_scores = sorted(valid_test, key = lambda x: (dir * x[0], -x[1]))
        return test_scores[0][-1]
