...
import torch
from xxlearn.cli import Const as C
from xxlearn.configs import Args
from xxlearn.visual import write_analysis
from xxlearn.utils import pickle_dump, CloudpickleWrapper, color_str, timeit
...
def DL2LD(lst, sort_key=None):
    res = [dict(zip(lst,t)) for t in zip(*lst.values())]
    if sort_key:
        res = sorted(res, key=lambda x: x[sort_key])

def LD2DL(dct, sort=False):
    if sort:
        return {k: sorted([dic[k] for dic in dct]) for k in dct[0]}
    else:
        return {k: [dic[k] for dic in dct] for k in dct[0]}
